...
const VerifyOtp = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        toast({ title: 'Invalid OTP', description: data.message || 'Verification failed', variant: 'destructive' });
        return;
      }
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" disabled={isLoading} className="w-full">{isLoading ? 'Verifying...' : 'Verify OTP'}</Button>
          </form>
        </CardContent>
      </Card>
    </div>
